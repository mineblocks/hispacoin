...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("HPC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("hispacoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non HPC unit input in lnReqAmount, we generate the URI with HPC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::HPC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
